...
class TTSIntegrationService(ITTSIntegrationService):
    ...
    def set_narration_enabled(self, enabled: bool) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Narration {'enabled' if enabled else 'disabled'} for current game session"
            )
            return True
        except Exception as e:
    ...
    def generate_tts_for_message(
        ...
        try:
            ...
            if audio_path:
                logger.info(
                    f"Generated TTS audio for message {message_id}: {audio_path}"
                )
                return audio_path
            else:
