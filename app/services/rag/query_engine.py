...
class RAGQueryEngineImpl:
    ...
    def __init__(self):
        ...
        self.known_spells = {
            ...
            'ice_knife': ['ice knife', 'iceknife', 'ice shard', 'frozen knife', 'ice blade'],
            'cure_wounds': ['cure wounds', 'cure wound', 'healing touch', 'heal wounds', 'mend wounds'],
            'healing_word': ['healing word', 'heal word', 'word of healing', 'healing phrase'],
            'vicious_mockery': ['vicious mockery', 'mockery', 'insult', 'verbal attack', 'cutting words'],
            'shield': ['shield', 'magic shield', 'barrier', 'protection'],
            'magic_missile': ['magic missile', 'missile', 'force missile', 'force dart'],
            'thunder_wave': ['thunder wave', 'thunderwave', 'sonic boom', 'thunder blast'],
            'eldritch_blast': ['eldritch blast', 'eldritch beam', 'warlock blast', 'eldritch ray']
        }
        ...
        self.spell_keywords = [
            'spell', 'magic', 'incantation', 'cantrip', 'enchantment', 'conjuration',
            'evocation', 'abjuration', 'divination', 'illusion', 'necromancy', 'transmutation'
        ]
    ...
    def analyze_action(self, action: str, game_state: Any) -> List[RAGQuery]:
        ...
        logger.debug(f"Generated {len(queries)} RAG queries for action type: {query_type.value}")
        return queries
    
    def _determine_query_type(self, action: str, game_state: Any) -> QueryType:
        ...
        if any(word in action for word in ['move', 'go', 'walk', 'travel', 'explore', 'enter', 'exit', 'door', 'path']):
    ...
    def _generate_spell_queries(self, action: str, game_state: Any) -> List[RAGQuery]:
        ...
        if spell_name:
            ...
            elif any(word in spell_name.lower() for word in ['heal', 'cure', 'restore']):
                queries.append(RAGQuery(
    ...
    def _generate_combat_queries(self, action: str, game_state: Any) -> List[RAGQuery]:
    ...
    def _generate_skill_queries(self, action: str, game_state: Any) -> List[RAGQuery]:
    ...
    def _generate_social_queries(self, action: str, game_state: Any) -> List[RAGQuery]:
    ...
    def _generate_exploration_queries(self, action: str, game_state: Any) -> List[RAGQuery]:
        """Generate queries for exploration actions."""
        queries = []
        
        # Current location context
        if hasattr(game_state, 'current_location'):
            location = game_state.current_location.get('name', '')
            if location:
                queries.append(RAGQuery(
    ...
    def _generate_general_queries(self, action: str, game_state: Any) -> List[RAGQuery]:
    ...
    def _extract_spell_name_enhanced(self, action: str) -> str:
        ...
        for pattern in self.spell_patterns:
            ...
            if match:
                spell_name = match.group(1).strip() if match.lastindex and match.lastindex >= 1 else match.group(0).strip()
                # Clean up common words
                spell_name = re.sub(r'\b(?:spell|at|on|the|a|an|against|with|my|his|her|their)\b', '', spell_name, flags=re.IGNORECASE).strip()
    ...
    def _extract_npc_name(self, action: str) -> str:
        ...
        for pattern in patterns:
            ...
            if match:
                ...
                if npc_name.lower() not in ['the', 'that', 'this', 'what', 'where', 'when', 'how']:
                    return npc_name
