...
class RAGServiceImpl(RAGService):
    ...
    def get_relevant_knowledge(self, action: str, game_state: Any) -> RAGResults:
        ...
        try:
            ...
            logger.debug(f"Generated {len(queries)} queries for action: '{action[:50]}...'")
            ...
            for query in queries:
                ...
                kb_types = query.knowledge_base_types if query.knowledge_base_types else None
                
                # For spell queries, prioritize exact spell name matches
                if query.query_type == QueryType.SPELL_CASTING and query.context.get("spell_name"):
                ...
                for query in queries:
                    ...
                    if query.context.get("creature") and result.source == "monsters":
                        ...
                        if creature_name_lower in result.content.lower():
                            result.relevance_score += 0.5  # Boost exact creature matches
            ...
            if final_results:
                logger.info(f"RAG retrieved {len(final_results)} results in {execution_time:.1f}ms")
                logger.debug(f"Top scores: {[round(r.relevance_score, 3) for r in final_results[:3]]}")
    ...
    def _ensure_campaign_knowledge(self, game_state: Any):
