    ...
    if level == 1:
        ...
        if use_average_rolls:
            ...
            max_hp = int(
                ...
                + (level - 1) * (avg_roll_per_level + constitution_modifier)
            )
        else:
...
def calculate_armor_class(
    ...
    equipped_armor: Optional[Any] = None,
    ...
    if equipped_armor:
        # Handle both dict and ArmorModel
        if isinstance(equipped_armor, dict):
            armor_type = equipped_armor.get("type", "light")
            base_ac = equipped_armor.get("base_ac", 10)
            max_dex_bonus = equipped_armor.get(
                "max_dex_bonus", equipped_armor.get("dex_max_bonus", None)
            )
        else:
            armor_type = equipped_armor.type
            base_ac = equipped_armor.base_ac
            max_dex_bonus = getattr(equipped_armor, "dex_max_bonus", None)
        ...
        if armor_type == "light":
            ...
            max_dex_bonus = max_dex_bonus if max_dex_bonus is not None else 2
...
def calculate_saving_throw_modifier(
    ability_score: int, level: int, is_proficient: bool = False
) -> int:
        ...
        if roll_type == "skill_check" and skill:
            ...
            if ability_key in stats:
                ...
                return calculate_saving_throw_modifier(
                    stats[ability_key], level, is_proficient
                )
