...
def _add_combatants_to_state(game_state: GameState, combat_update: CombatStartUpdate, game_manager):
    ...
    for npc_data in combat_update.combatants:
        ...
        if not npc_id:
            logger.error(f"NPC data missing 'id' in combat_start: {npc_data}. Skipping.")
            continue
        ...
        combat.monster_stats[npc_id] = {
            "name": npc_name, "hp": initial_hp, "ac": npc_data.get("ac", 10),
            "conditions": [], "initial_hp": initial_hp,
...
def end_combat(game_state: GameState, update: CombatEndUpdate):
...
def remove_combatant_from_state(game_state: GameState, combatant_id_to_remove: str, details: Optional[Dict], game_manager):
    ...
    removed_combatant_name = combat.combatants[removed_index].name
...
def check_and_end_combat_if_over(game_state: GameState, game_manager):
    ...
    active_npcs_found = any(
        not c.is_player and
        game_state.combat.monster_stats.get(c.id, {}).get("hp", 0) > 0 and
        "Defeated" not in game_state.combat.monster_stats.get(c.id, {}).get("conditions", [])
    ...
    if not active_npcs_found:
        ...
        end_combat(game_state, CombatEndUpdate(type="combat_end", details={"reason": "No active enemies remaining"}))
...
def apply_hp_change(game_state: GameState, update: HPChangeUpdate, resolved_char_id: str, game_manager):
    ...
    if player:
        ...
        logger.info(f"Updated HP for {player.name} ({resolved_char_id}): {old_hp} -> {player.current_hp} (Delta: {delta})")
        if player.current_hp == 0:
            logger.info(f"{player.name} has dropped to 0 HP!")
    elif game_state.combat.is_active and resolved_char_id in game_state.combat.monster_stats:
...
def apply_condition_update(game_state: GameState, update: ConditionUpdate, resolved_char_id: str, game_manager):
    ...
    if player:
        ...
        monster = game_state.combat.monster_stats[resolved_char_id]
        monster.setdefault("conditions", [])
        target_conditions_list = monster["conditions"]
    else:
...
def apply_quest_update(game_state: GameState, update: QuestUpdate):
    ...
    if update.status and quest.status != update.status:
        ...
        updated = True
    if update.details:
        if not isinstance(quest.details, dict): quest.details = {}
...
def _get_target_ids_for_update(game_state: GameState, character_id_field: str, game_manager) -> List[str]:
    ...
    resolved_id = game_manager.find_combatant_id_by_name_or_id(character_id_field)
    if resolved_id:
        return [resolved_id]
    ...
    for update_obj in updates:
        ...
        try:
            ...
            # Updates that don't iterate or have specific targetting logic
                ...
                if game_state.combat.is_active:
                    end_combat(game_state, update_obj)
                ...
                apply_quest_update(game_state, update_obj) # Quest updates are global by quest_id
            else:
