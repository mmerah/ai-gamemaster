...
class PromptBuilder:
    ...
    def build_ai_prompt_context(self, game_state: GameStateModel, handler_self: Any, 
        ...
        world_lore_formatted = self.format_list_context("World Lore", game_state.world_lore)
        ...
        event_summary_formatted = self.format_list_context("Event Summary", game_state.event_summary)
        ...
        if static_context_parts:
            ...
            static_context_messages.append(HumanMessage(content=f"CONTEXT INJECTION:\n{static_context_content}"))
        ...
        dynamic_context_messages = [HumanMessage(content=f"CURRENT STATUS:\n{dynamic_context_content}")]
        ...
        fixed_token_count = sum(self._calculate_message_tokens(msg) for msg in fixed_messages)
        ...
        if main_history_messages and remaining_budget > 0:
            try:
                ...
                trimmed_main_history = trim_messages(
                    main_history_messages,
                ...
                logger.warning(f"Error trimming messages with LangChain, falling back to manual trimming: {e}")
                # Fallback to manual trimming
                trimmed_main_history = []
                current_tokens = 0
                for msg in reversed(main_history_messages):
                    msg_tokens = self._calculate_message_tokens(msg)
                    if current_tokens + msg_tokens <= remaining_budget:
                        trimmed_main_history.insert(0, msg)
        ...
        total_tokens = sum(self._calculate_message_tokens(msg) for msg in final_messages)
        
        # Log prompt info
        history_truncated = len(main_history_messages) - len(trimmed_main_history)
        logger.debug(f"Built AI prompt with {len(final_dict_messages)} messages ({total_tokens} tokens)")
        if history_truncated > 0:
            logger.debug(f"Truncated {history_truncated} older history messages due to token budget")
        
        # Log individual messages
        logger.debug("=================================================================================")
        for i, msg in enumerate(final_dict_messages):
