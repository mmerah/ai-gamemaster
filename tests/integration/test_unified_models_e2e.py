...
# Import removed - EventData not needed
...
@pytest.fixture
def app_with_temp_dirs(temp_save_dir, tmp_path):
    ...
    app = create_app({
        'GAME_STATE_REPO_TYPE': 'file',  # Use file type to actually test file operations
        'TTS_PROVIDER': 'disabled',
        'RAG_ENABLED': False,
        'TESTING': True,
        'CAMPAIGNS_DIR': str(tmp_path),  # FileGameStateRepository will add campaigns/ subdirectory
        'CHARACTER_TEMPLATES_DIR': str(temp_save_dir['character_templates']),
        'CAMPAIGN_TEMPLATES_DIR': str(temp_save_dir['campaign_templates'])
    })
...
class TestUnifiedModelsE2E:
    ...
    def test_complete_flow_with_unified_models(self, app_with_temp_dirs, temp_save_dir):
        ...
        from app.game.unified_models import BaseStatsModel, ProficienciesModel, ItemModel, TraitModel
        ...
        warrior_template = CharacterTemplateModel(
            ...
            feats=[TraitModel(name="Tough", description="Your hit point maximum increases by an amount equal to twice your level.")],
            starting_equipment=[
                ItemModel(id="chainmail", name="Chainmail", description="Heavy armor providing AC 16", quantity=1),
                ItemModel(id="shield", name="Shield", description="+2 AC bonus", quantity=1),
                ItemModel(id="battleaxe", name="Battleaxe", description="1d8 slashing damage", quantity=1),
                ItemModel(id="handaxe", name="Handaxe", description="1d6 slashing damage, light, thrown", quantity=2),
                ItemModel(id="explorers_pack", name="Explorer's Pack", description="Includes bedroll, mess kit, tinderbox, torches, rations, waterskin, rope", quantity=1)
            ],
        ...
        mage_template = CharacterTemplateModel(
            ...
            personality_traits=["I use polysyllabic words that convey the impression of great erudition.",
            ...
            feats=[],
            starting_equipment=[
                ItemModel(id="quarterstaff", name="Quarterstaff", description="1d6 bludgeoning damage, versatile (1d8)", quantity=1),
                ItemModel(id="component_pouch", name="Component Pouch", description="Contains material components for spells", quantity=1),
                ItemModel(id="scholars_pack", name="Scholar's Pack", description="Includes book, ink, parchment, small knife", quantity=1),
                ItemModel(id="spellbook", name="Spellbook", description="Contains your wizard spells", quantity=1)
            ],
        ...
        # Create campaign instance metadata
        campaign_instance = CampaignInstanceModel(
            id="instance_" + str(uuid.uuid4())[:8],
            name="Our Phandelver Adventure",
            template_id=campaign_template.id,
            character_ids=[t.id for t in character_templates],
            current_location=campaign_template.starting_location.name,
            event_log_path=f"campaigns/instance_test/event_log.json",
            created_date=datetime.now(timezone.utc),
            last_played=datetime.now(timezone.utc)
        )
        
        # Just start campaign from template without mocking
        game_state_dict = campaign_service.start_campaign_from_template(
            template_id=campaign_template.id,
            instance_name="Our Phandelver Adventure",
            party_character_ids=[t.id for t in character_templates]
        )
        
        # Remove campaign-level fields that don't belong in GameStateModel
        campaign_fields = ['narration_enabled', 'tts_voice']
        for field in campaign_fields:
            game_state_dict.pop(field, None)
        
        # Convert dict to GameStateModel
        game_state = GameStateModel(**game_state_dict)
        ...
        assert game_state.current_location.name == campaign_template.starting_location.name
        ...
        # Create a mock AI service for gameplay
        ...
        with patch.object(game_state_repo, 'get_game_state', return_value=game_state):
            ...
            location_events = [e for e in event_recorder.events if e.event_type == "location_changed"]
            assert len(location_events) > 0
            location_event_data = location_events[-1]
            ...
            mock_ai_service.add_response(AIResponse(
                narrative="Four goblins jump out from behind the wagons! Roll for initiative!",
                reasoning="Ambush encounter",
                game_state_updates=[
                    CombatStartUpdate(
                        combatants=[
                            InitialCombatantData(id="goblin1", name="Goblin Scout", hp=7, ac=15),
                            InitialCombatantData(id="goblin2", name="Goblin Archer", hp=7, ac=15),
                            InitialCombatantData(id="goblin3", name="Goblin Warrior", hp=9, ac=13),
                            InitialCombatantData(id="goblin4", name="Goblin Leader", hp=12, ac=16)
                        ]
                    )
                ],
                dice_requests=[
            ...
            response = client.post("/api/player_action", json={
                "action_type": "free_text",
                "value": "We investigate the overturned wagons."
            })
            assert response.status_code == 200
            
            # Verify combat started event with unified models
            combat_events = [e for e in event_recorder.events if e.event_type == "combat_started"]
            assert len(combat_events) > 0
            combat_event = combat_events[-1]
            
            # The event should have properly typed combatants
            if hasattr(combat_event, 'data') and hasattr(combat_event.data, 'combatants'):
                for combatant in combat_event.data.combatants:
                    # Should be able to access as CombatantModel attributes
                    assert hasattr(combatant, 'id')
                    assert hasattr(combatant, 'name')
                    assert hasattr(combatant, 'current_hp')
            
            # Test quest addition
            event_recorder.clear()
            
            mock_ai_service.add_response(AIResponse(
                narrative="You find a note on one of the goblins mentioning their hideout location.",
                reasoning="Quest discovery",
                game_state_updates=[
                    # For adding a new quest, we'd typically handle this differently
                    # The QuestUpdate model is for updating existing quests
                    # Here we'll simulate updating a quest status
                    QuestUpdate(
                        quest_id="escort_wagon",
                        status="completed",
                        details={"found_clue": "Goblin hideout location"}
                    )
                ]
            ))
            ...
            response = client.post("/api/player_action", json={
                "action_type": "free_text",
                "value": "We search the goblin bodies for clues."
            })
            assert response.status_code == 200
            
            # Verify quest updated event
            quest_events = [e for e in event_recorder.events if e.event_type == "quest_updated"]
            assert len(quest_events) > 0
            
        # ========== PHASE 6: Test Save/Load with Unified Models ==========
        print("\n6. Testing save/load functionality with unified models...")
        
        # Save current game state (the repository should create directories automatically)
        game_state_repo.save_game_state(game_state)
        
        # The FileGameStateRepository uses base_save_dir (tmp_path) + "campaigns" + campaign_id
        # So the save path is: tmp_path / "campaigns" / game_state.campaign_id / "active_game_state.json"
        from pathlib import Path
        save_path = Path(app.config['CAMPAIGNS_DIR']) / "campaigns" / game_state.campaign_id / "active_game_state.json"
        ...
        loaded_state = game_state_repo.load_campaign_state(game_state.campaign_id)
        assert isinstance(loaded_state, GameStateModel)
        assert loaded_state.campaign_id == game_state.campaign_id
        assert loaded_state.current_location.name == game_state.current_location.name
        assert len(loaded_state.party) == len(game_state.party)
        
        print("\n✓ Complete unified models flow test passed!")
        
    def test_game_state_snapshot_event_typing(self, app_with_temp_dirs):
        ...
        with patch.object(game_state_repo, 'get_game_state', return_value=game_state):
            ...
            snapshot_events = [e for e in event_recorder.events if e.event_type == "game_state_snapshot"]
    ...
    def test_character_instance_template_integration(self, app_with_temp_dirs, temp_save_dir):
        ...
        from app.game.unified_models import BaseStatsModel, ProficienciesModel
        ...
        instance_data = char_factory.from_template(template, "test_campaign")
        
        # Create CharacterInstanceModel
        instance = CharacterInstanceModel(**instance_data)
        ...
        with patch.object(game_state_repo, 'get_game_state', return_value=mock_game_state):
            with patch.object(char_service.template_repo, 'get_template', return_value=template):
