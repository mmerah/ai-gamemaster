...
class MockAIService:
    ...
    def get_response(self, messages, model=None, game_state=None):
        """Return the next queued response."""
        if self.call_index >= len(self.responses):
            raise ValueError(f"No response defined for call {self.call_index}")
        response = self.responses[self.call_index]
        self.call_index += 1
        return response


def setup_mock_ai_service_patching(container, mock_ai_service):
    """
    Patch the _get_ai_service method on all event handlers to ensure they use our mock.
    
    Returns:
        list: List of patchers that need to be stopped in the finally block
    """
    game_event_manager = container.get_game_event_manager()
    handlers_to_patch = [
        game_event_manager.player_action_handler,
        game_event_manager.dice_submission_handler,
        game_event_manager.next_step_handler,
        game_event_manager.retry_handler
    ]
    
    # Store patchers to stop them later
    ai_service_patchers = []
    for handler_instance in handlers_to_patch:
        if handler_instance:
            patcher = patch.object(handler_instance, '_get_ai_service', return_value=mock_ai_service)
            ai_service_patchers.append(patcher)
            patcher.start()
    
    return ai_service_patchers


def verify_event_system_integrity(event_recorder):
    """
    ...
    for i, event in enumerate(all_events):
        ...
        if i > 0:
            assert event.timestamp >= all_events[i-1].timestamp, f"Event {i} timestamp out of order"
            assert event.sequence_number > all_events[i-1].sequence_number, f"Event {i} sequence out of order"
    ...
    return {
        ...
        'sequence_range': (all_events[0].sequence_number, all_events[-1].sequence_number) if all_events else (0, 0)
    }


def verify_required_event_types(event_types, test_name):
    """
    ...
    for event_type in core_events:
        assert event_type in event_types, f"{test_name}: Missing required event type {event_type}"
    
    return True


def _verify_event_payload_integrity(all_events):
    """
    ...
    if 'CombatantHpChangedEvent' in events_by_type:
        for hp_event in events_by_type['CombatantHpChangedEvent']:
            ...
            if hasattr(hp_event, 'old_hp') and hasattr(hp_event, 'new_hp') and hasattr(hp_event, 'change_amount'):
                expected_new_hp = max(0, hp_event.old_hp + hp_event.change_amount)  # HP cannot go below 0
                assert hp_event.new_hp == expected_new_hp, f"HP math inconsistent: max(0, {hp_event.old_hp} + {hp_event.change_amount}) = {expected_new_hp} != {hp_event.new_hp}"
    ...
    if 'CombatantStatusChangedEvent' in events_by_type:
        for condition_event in events_by_type['CombatantStatusChangedEvent']:
            assert hasattr(condition_event, 'combatant_id'), "Condition event missing combatant_id"
            assert (hasattr(condition_event, 'added_conditions') or 
                   hasattr(condition_event, 'removed_conditions')), "Condition event missing condition changes"
    
    # Verify Turn Advancement Events
    if 'TurnAdvancedEvent' in events_by_type:
        for turn_event in events_by_type['TurnAdvancedEvent']:
            assert hasattr(turn_event, 'new_combatant_id'), "Turn event missing new_combatant_id"
            assert hasattr(turn_event, 'new_combatant_name'), "Turn event missing new_combatant_name"
            assert hasattr(turn_event, 'round_number'), "Turn event missing round_number"
    ...
    if 'CombatStartedEvent' in events_by_type:
        ...
        for combat_event in combat_started_events:
            assert hasattr(combat_event, 'combatants'), "Combat started event missing combatants"
    ...
    if 'PlayerDiceRequestAddedEvent' in events_by_type:
        for dice_event in events_by_type['PlayerDiceRequestAddedEvent']:
            assert hasattr(dice_event, 'character_id'), "Dice request missing character_id"
            assert hasattr(dice_event, 'roll_type'), "Dice request missing roll_type"
            assert hasattr(dice_event, 'dice_formula'), "Dice request missing dice_formula"
    ...
    if 'BackendProcessingEvent' in events_by_type:
        ...
        start_events = [e for e in processing_events if getattr(e, 'is_processing', False)]
        end_events = [e for e in processing_events if not getattr(e, 'is_processing', True)]
...
def verify_initiative_sequence(all_events, expected_combatant_count):
    """
    Verify that initiative sequence events follow the expected pattern.
    """
    initiative_set_events = [e for e in all_events if type(e).__name__ == 'CombatantInitiativeSetEvent']
    initiative_order_events = [e for e in all_events if type(e).__name__ == 'InitiativeOrderDeterminedEvent']
    turn_advanced_events = [e for e in all_events if type(e).__name__ == 'TurnAdvancedEvent']
    
    # Should have initiative set events for each combatant (or at least some if NPCs auto-roll)
    if initiative_set_events:
        assert len(initiative_set_events) <= expected_combatant_count, f"Too many initiative set events: {len(initiative_set_events)} > {expected_combatant_count}"
    
    # Should have exactly one initiative order determination
    if initiative_order_events:
        assert len(initiative_order_events) == 1, f"Expected 1 initiative order event, got {len(initiative_order_events)}"
        
        order_event = initiative_order_events[0]
        assert hasattr(order_event, 'ordered_combatants'), "Initiative order event missing ordered_combatants"
    
    # Should have at least one turn advancement (to first combatant)
    if turn_advanced_events:
        assert len(turn_advanced_events) >= 1, f"Expected at least 1 turn advancement, got {len(turn_advanced_events)}"
    ...
    if initiative_set_events and initiative_order_events:
        ...
        assert last_init_set < first_order, "Initiative order determined before all initiatives set"
    ...
    if initiative_order_events and turn_advanced_events:
        ...
        assert last_order < first_turn, "Turn advanced before initiative order determined"
...
def sanitize_event_for_golden(event_dict: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if 'character_id' in sanitized and isinstance(sanitized['character_id'], str) and len(sanitized['character_id']) == 36 and '-' in sanitized['character_id']:
        # This looks like a UUID, replace it
        sanitized['character_id'] = f"CHARACTER_UUID_{sanitized.get('character_name', 'UNKNOWN')}"
    
    if 'combatant_id' in sanitized and isinstance(sanitized['combatant_id'], str) and len(sanitized['combatant_id']) == 36 and '-' in sanitized['combatant_id']:
        # This looks like a UUID, replace it
        sanitized['combatant_id'] = f"COMBATANT_UUID_{sanitized.get('combatant_name', 'UNKNOWN')}"
    
    # Handle cleared_request_ids list
    if 'cleared_request_ids' in sanitized and isinstance(sanitized['cleared_request_ids'], list):
        sanitized['cleared_request_ids'] = ['CLEARED_UUID' for _ in sanitized['cleared_request_ids']]
    ...
    if 'combatants' in sanitized and isinstance(sanitized['combatants'], list):
        for i, combatant in enumerate(sanitized['combatants']):
            if isinstance(combatant, dict) and 'id' in combatant:
                ...
                if isinstance(combatant['id'], str) and len(combatant['id']) == 36 and '-' in combatant['id']:
                    combatant['id'] = f"COMBATANT_UUID_{combatant.get('name', f'INDEX_{i}')}"
    
    return sanitized


def save_golden_event_log(event_recorder: EventRecorder, test_name: str, golden_dir: str = None):
    """
...
def compare_with_golden(event_recorder: EventRecorder, test_name: str, golden_dir: str = None) -> bool:
    """
    ...
    if not os.path.exists(golden_path):
        print(f"⚠️  No golden file found at {golden_path}. Run with UPDATE_GOLDEN=1 to create.")
        return False
    ...
    if len(golden_events) != len(current_events):
        print(f"❌ Event count mismatch: golden={len(golden_events)}, current={len(current_events)}")
        return False
...
def app(temp_saves_dir):
    ...
    app = create_app({
        'GAME_STATE_REPO_TYPE': 'memory',  # Use memory to avoid file creation
        'TTS_PROVIDER': 'disabled',
        'RAG_ENABLED': False,
        'TESTING': True,
        'CAMPAIGNS_DIR': f'{temp_saves_dir}/campaigns',  # Use temp dir if needed
...
def golden_test(request):
    ...
    """
    def _golden_test(event_recorder, test_name):
        if os.environ.get('UPDATE_GOLDEN') == '1':
            golden_path = save_golden_event_log(event_recorder, test_name)
            print(f"📝 Updated golden file: {golden_path}")
        else:
            assert compare_with_golden(event_recorder, test_name), f"Events don't match golden file for {test_name}"
