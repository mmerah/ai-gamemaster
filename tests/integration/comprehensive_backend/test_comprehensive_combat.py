...
def test_comprehensive_dnd_combat(app, client, mock_ai_service, event_recorder, container, full_party, golden_test):
    ...
    def mock_npc_roll(character_id, roll_type, dice_formula, **kwargs):
    ...
    dice_patcher = patch.object(container.get_dice_service(), 'perform_roll', side_effect=mock_npc_roll)
    dice_patcher.start()
    ...
    try:
        ...
        mock_ai_service.add_response(AIResponse(
            ...
            game_state_updates=[
                CombatStartUpdate(
                    combatants=[
                        InitialCombatantData(id="kobold_sorcerer", name="Kobold Draconic Sorcerer", hp=27, ac=13),
                        InitialCombatantData(id="kobold_warrior1", name="Kobold Warrior", hp=12, ac=13),
                        InitialCombatantData(id="kobold_warrior2", name="Kobold Warrior", hp=12, ac=13),
                        InitialCombatantData(id="kobold_archer", name="Kobold Archer", hp=10, ac=12),
                        InitialCombatantData(id="drake", name="Guard Drake", hp=52, ac=14)
                    ]
                )
            ],
            dice_requests=[
        ...
        assert len(combat_events[0].combatants) == 9  # 4 party + 5 enemies
        ...
        client.post("/api/submit_rolls", json=[{
            ...
            "roll_type": "attack",
            "dice_formula": "1d20+8",
            "dice_notation": "1d20+8",
            "dice_results": [(20, 18)],  # Natural 20!
            "total": 26
        }])
        ...
        mock_ai_service.add_response(AIResponse(
            ...
            game_state_updates=[
                HPChangeUpdate(
                    ...
                    value=-39,
                    details={"source": "Rogue's critical sneak attack", "damage_type": "piercing"}
                ),
                ConditionUpdate(
                    type="condition_add",
                    character_id="kobold_sorcerer",
            ...
            end_turn=True,
            next_turn={"combatant_id": "kobold_archer"}
        ))
        ...
        mock_ai_service.add_response(AIResponse(
            ...
            game_state_updates=[
                HPChangeUpdate(
                    ...
                    details={"source": "Kobold archer's arrow", "damage_type": "piercing"}
                )
            ],
            end_turn=True,
        ...
        client.post("/api/submit_rolls", json=[{
            ...
            "dice_formula": "6d6",
            "dice_notation": "6d6",
            "dice_results": [(6, 4), (6, 5), (6, 3), (6, 6), (6, 2), (6, 4)],
            "total": 24
        }])
        ...
        game_state_updates=[
            ...
            HPChangeUpdate(
                ...
                details={"source": "Fireball (saved)", "damage_type": "fire"}
            ),
            ConditionUpdate(
                type="condition_add",
                character_id="kobold_warrior2",
        ...
        client.post("/api/submit_rolls", json=[{
            ...
            "dice_formula": "8d6",
            "dice_notation": "8d6",
            "dice_results": [(6, 4), (6, 5), (6, 3), (6, 6), (6, 2), (6, 4), (6, 5), (6, 3)],
            "total": 32
        }])
        ...
        game_state_updates=[
            HPChangeUpdate(
                character_id="drake",
                value=-20,
                details={"source": "Combined party attacks", "damage_type": "mixed"}
            ),
            ConditionUpdate(
                type="condition_add",
                character_id="drake",
        ...
        game_state_updates=[
            InventoryUpdate(
                ...
                details={"description": "3rd level spell scroll"}
            ),
            InventoryUpdate(
                type="inventory_add",
                character_id="fighter",
                value="Potion of Fire Resistance",
                details={"description": "Grants resistance to fire damage for 1 hour"}
            ),
            QuestUpdate(
                quest_id="dragon_lair",
                details={"progress": "Defeated kobold cultists, found map to inner sanctum"}
            )
        ]
        ))
        ...
        assert len(combat_events[0].combatants) == 9, f"Expected 9 combatants, got {len(combat_events[0].combatants)}"
        ...
        # Verify dice system
        dice_request_events = event_recorder.get_events_of_type(PlayerDiceRequestAddedEvent)
        assert len(dice_request_events) >= 4, f"Expected at least 4 dice requests, got {len(dice_request_events)}"
        ...
        for quest_event in quest_events:
            ...
            assert quest_event.quest_id == "dragon_lair", f"Unexpected quest_id: {quest_event.quest_id}"
        ...
        for item_event in item_events:
            ...
            print(f"      ✅ Item '{item_event.item_name}' added to {item_event.character_id}")

        # ========== COMBAT MECHANICS VERIFICATION ==========

        hp_events = event_recorder.get_events_of_type(CombatantHpChangedEvent)
        condition_events = event_recorder.get_events_of_type(CombatantStatusChangedEvent)
        ...
        for condition_event in condition_events:
            ...
            assert hasattr(condition_event, 'added_conditions') or hasattr(condition_event, 'removed_conditions'), "Condition event missing condition changes"
        ...
        print(f"   ⚔️  Combat Participants: {len(combat_events[0].combatants) if combat_events else 0}")
        ...
        print(f"   📝 Event Types ({len(event_stats['event_types'])}): {sorted(event_stats['event_types'])}")
        print(f"   ⏱️  Total Duration: {event_stats['last_timestamp'] - event_stats['first_timestamp'] if event_stats['first_timestamp'] else 0}ms")
    finally:
        # Always stop the dice patcher
        dice_patcher.stop()
