...
_module_rag_service: Optional[RAGServiceImpl] = None


def get_module_rag_service() -> RAGServiceImpl:
...
class MockGameState:
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        self.current_location = kwargs.get(
            "current_location",
            LocationModel(
...
def rag_service() -> RAGServiceImpl:
...
class TestRAGSpellCasting:
    ...
    def test_ice_knife_on_goblin(
        self, rag_service: RAGServiceImpl, mock_game_state: MockGameState
    ) -> None:
        """Test the primary scenario: Cast Ice Knife on Goblin."""
        action = "Cast Ice Knife on Goblin"

        results = rag_service.get_relevant_knowledge(action, mock_game_state)  # type: ignore[arg-type]
        ...
        spell_results = [
            ...
            if r.source == "spells" and "ice knife" in r.content.lower()
        ]
        ...
        creature_results = [
            r
            for r in results.results
            if r.source == "monsters" and "goblin" in r.content.lower()
        ]
        ...
        assert len(unique_content) == len(results.results), (
    ...
    def test_fireball_spell(
        ...
        spell_results = [
            r
            for r in results.results
            if r.source == "spells" and "fireball" in r.content.lower()
        ]
        assert len(spell_results) >= 1, "Should return Fireball spell information"

    def test_healing_word_spell(
        self, rag_service: RAGServiceImpl, mock_game_state: MockGameState
    ) -> None:
        ...
        spell_results = [
            ...
            for r in results.results
            if r.source == "spells"
            and ("healing word" in r.content.lower() or "heal" in r.content.lower())
        ]
    ...
    def test_unknown_spell(
        ...
        assert results.total_queries > 0, (
            "Should generate queries even for unknown spells"
        )
...
class TestRAGCombat:
    ...
    def test_attack_orc(self, rag_service: RAGServiceImpl) -> None:
        ...
        creature_results = [
            r
            for r in results.results
            if r.source == "monsters" and "orc" in r.content.lower()
        ]
        ...
        rules_results = [
            ...
            for r in results.results
            if r.source == "rules"
            and ("attack" in r.content.lower() or "combat" in r.content.lower())
        ]
        assert len(rules_results) >= 1, "Should return combat rules information"

    def test_grapple_dragon(self, rag_service: RAGServiceImpl) -> None:
        ...
        creature_results = [
            r
            for r in results.results
            if r.source == "monsters" and "dragon" in r.content.lower()
        ]
...
class TestRAGSkillChecks:
    ...
    def test_perception_check(
        ...
        rules_results = [
            ...
            for r in results.results
            if r.source == "rules"
            and ("perception" in r.content.lower() or "skill" in r.content.lower())
        ]
    ...
    def test_stealth_action(
        ...
        rules_results = [
            r
            for r in results.results
            if r.source == "rules" and "stealth" in r.content.lower()
        ]
...
class TestRAGSocialInteraction:
    ...
    def test_persuasion_attempt(
        ...
        rules_results = [
            ...
            for r in results.results
            if r.source == "rules"
            and ("persuasion" in r.content.lower() or "social" in r.content.lower())
        ]
...
class TestRAGQueryGeneration:
    ...
    def test_spell_query_generation(
        self, rag_service: RAGServiceImpl, mock_game_state: MockGameState
    ) -> None:
        """Test query generation for spells."""
        action = "Cast Ice Knife on Goblin Cook"

        queries = rag_service.analyze_action(action, mock_game_state)  # type: ignore[arg-type]
    ...
    def test_combat_query_generation(self, rag_service: RAGServiceImpl) -> None:
...
class TestRAGPerformance:
    ...
    def test_response_time(
        ...
        assert results.execution_time_ms < 5000, (
            f"RAG took too long: {results.execution_time_ms}ms"
        )
    ...
    def test_empty_action(
        ...
        results = rag_service.get_relevant_knowledge("", mock_game_state)  # type: ignore[arg-type]
        assert isinstance(results, RAGResults), (
            "Should return RAGResults for empty action"
        )

        results = rag_service.get_relevant_knowledge(None, mock_game_state)  # type: ignore[arg-type]
    ...
    def test_very_long_action(
        ...
        results = rag_service.get_relevant_knowledge(long_action, mock_game_state)  # type: ignore[arg-type]

        assert isinstance(results, RAGResults), "Should handle long actions"
        assert results.execution_time_ms < 10000, (
            "Should complete long actions in reasonable time"
        )
...
class TestRAGConfiguration:
    ...
    def test_filtering_configuration(
        self, rag_service: RAGServiceImpl, mock_game_state: MockGameState
    ) -> None:
...
class TestRAGEdgeCases:
    ...
    def test_special_characters(
        ...
        assert len(spell_results) >= 1, (
            "Should still extract spell despite special chars"
        )
...
class TestRAGContextPersistence:
    ...
    def test_context_persistence_during_dice_submission(self) -> None:
        ...
        # Create a proper GameStateModel using test helper
        from tests.test_helpers import create_mock_game_state
        ...
        mock_results.format_for_prompt.return_value = (
            "SPELL CONTEXT: Ice Knife spell details here..."
        )
        ...
        chat_history: list[ChatMessageModel] = [
            ChatMessageModel(
                id=str(uuid.uuid4()),
                role="user",
                content="I cast Ice Knife on the goblin",
                timestamp=datetime.now(timezone.utc).isoformat(),
            ),
            ChatMessageModel(
                id=str(uuid.uuid4()),
                role="assistant",
                content="Roll an attack roll",
                timestamp=datetime.now(timezone.utc).isoformat(),
            ),
            ChatMessageModel(
                id=str(uuid.uuid4()),
        ...
        # Note: MockGameState is used for testing, using type: ignore for incompatible type
        context1 = rag_context_builder.get_rag_context_for_prompt(
            game_state,
            mock_rag_service,
            "I cast Ice Knife on the goblin",
            chat_history,
    ...
    def test_context_clearing_on_new_action(self) -> None:
        ...
        rag_context_builder.clear_stored_rag_context(game_state)  # type: ignore[arg-type]

        # Verify context was cleared
        assert game_state._last_rag_context is None

    def test_context_clearing_on_combat_end(self) -> None:
        ...
        combat_end_update = CombatEndUpdateModel(reason="All enemies defeated")
