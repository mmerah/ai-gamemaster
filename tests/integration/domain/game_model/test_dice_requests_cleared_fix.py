...
class TestPlayerDiceRequestsClearedFix:
    ...
    @pytest.fixture
    def container(self, app: FastAPI) -> ServiceContainer:
    ...
    def test_only_submitted_dice_requests_are_cleared(
        self, app: FastAPI, mock_ai_service: MockAIServiceProtocol
    ) -> None:
        """
        ...
        """
        # Use the centralized client fixture instead
        ...
        with TestClient(app) as client:
            ...
            action_request = PlayerActionRequest(
                action_type="free_text", value="We search the room"
            )
            ...
            rolls_request = SubmitRollsRequest(rolls=[fighter_roll])
            response = client.post(
                "/api/submit_rolls",
    ...
    def test_multiple_submissions_clear_correctly(
        ...
        with TestClient(app) as client:
            ...
            action_request = PlayerActionRequest(
                action_type="free_text", value="Attack!"
            )
            response = client.post(
                "/api/player_action",
                json=action_request.model_dump(mode="json", exclude_unset=True),
