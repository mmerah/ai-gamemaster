...
class TestCharacterTemplateSerialization(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.test_character = CharacterTemplateModel(
            ...
            # Stats
            base_stats=BaseStatsModel(STR=14, DEX=16, CON=16, INT=10, WIS=14, CHA=8),
            # Proficiencies
            ...
            starting_equipment=[
                ...
                ItemModel(
                    id="longbow", name="Longbow", description="1d8 piercing", quantity=1
                ),
                ItemModel(
                    id="arrows", name="Arrows", description="Ammunition", quantity=40
                ),
                ItemModel(
                    id="shortsword",
                    name="Shortsword",
    ...
    def test_model_to_json_serialization(self) -> None:
        ...
        json_str = json.dumps(
            character_dict, default=str
        )  # default=str handles datetime
        ...
        arrows: Optional[Dict[str, Any]] = next(
            (item for item in parsed["starting_equipment"] if item["name"] == "Arrows"),
            None,
        )
        self.assertIsNotNone(arrows)
        assert arrows is not None  # Type guard for mypy
        self.assertEqual(arrows["quantity"], 40)

    def test_json_to_model_deserialization(self) -> None:
        ...
        json_data = {
            ...
            "class_features": [
                {
                    ...
                    "description": "d8 inspiration dice",
                    "level_acquired": 1,
                },
                {
                ...
                {
                    "name": "Cutting Words",
                    "description": "Reduce enemy roll with inspiration",
                    "level_acquired": 3,
                },
            ],
            "feats": [
                {
                    "name": "Actor",
                    "description": "+1 CHA, advantage on deception and performance",
                }
            ],
            "spells_known": [
                "Charm Person",
            ...
            "portrait_path": None,
            "personality_traits": [
                "I change my mood as quickly as I change keys in a song"
            ],
    ...
    def test_round_trip_serialization(self) -> None:
        ...
        self.assertEqual(
            len(character2.proficiencies.skills),
            len(self.test_character.proficiencies.skills),
        )
        self.assertEqual(
            len(character2.racial_traits), len(self.test_character.racial_traits)
        )
        self.assertEqual(
            character2.racial_traits[0].name, self.test_character.racial_traits[0].name
        )

        # Compare equipment
        self.assertEqual(
            len(character2.starting_equipment),
            len(self.test_character.starting_equipment),
        )
        arrows1: Optional[ItemModel] = next(
            (
                item
                for item in self.test_character.starting_equipment
                if item.name == "Arrows"
            ),
            None,
        )
        arrows2: Optional[ItemModel] = next(
            (item for item in character2.starting_equipment if item.name == "Arrows"),
            None,
        )
        self.assertIsNotNone(arrows1)
        self.assertIsNotNone(arrows2)
        assert arrows1 is not None and arrows2 is not None  # Type guards for mypy
    ...
    def test_empty_lists_serialization(self) -> None:
