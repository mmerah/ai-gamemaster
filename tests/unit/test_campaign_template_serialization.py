...
class TestCampaignTemplateSerializationTest(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.test_campaign = CampaignTemplateModel(
            ...
            starting_location=LocationModel(
                name="Test City", description="A test location"
            ),
    ...
    def test_json_round_trip(self) -> None:
        ...
        self.assertEqual(
            restored_campaign.campaign_goal, self.test_campaign.campaign_goal
        )
        self.assertEqual(
            restored_campaign.starting_level, self.test_campaign.starting_level
        )
        ...
        self.assertEqual(
            restored_campaign.starting_location.name,
            self.test_campaign.starting_location.name,
        )
        self.assertEqual(
            restored_campaign.starting_location.description,
            self.test_campaign.starting_location.description,
        )

        # Verify NPCs
        self.assertEqual(
            len(restored_campaign.initial_npcs), len(self.test_campaign.initial_npcs)
        )
        ...
        # Verify Quests
        self.assertEqual(
            len(restored_campaign.initial_quests),
            len(self.test_campaign.initial_quests),
        )
        ...
        self.assertEqual(
            restored_campaign.allowed_races, self.test_campaign.allowed_races
        )
        self.assertEqual(
            restored_campaign.allowed_classes, self.test_campaign.allowed_classes
        )
        ...
        # Verify gold range
        self.assertIsNotNone(restored_campaign.starting_gold_range)
        assert restored_campaign.starting_gold_range is not None  # Type guard for mypy
    ...
    def test_datetime_serialization(self) -> None:
        ...
        CampaignTemplateModel(**parsed_dict)
        # Dates will be strings after JSON round trip - this is expected

    def test_optional_fields_serialization(self) -> None:
        ...
        minimal_campaign = CampaignTemplateModel(
            ...
            starting_location=LocationModel(
                name="Start", description="Starting location"
            ),
    ...
    def test_empty_collections_serialization(self) -> None:
    ...
    def test_large_nested_structures(self) -> None:
        ...
        large_campaign = CampaignTemplateModel(
            ...
            starting_location=LocationModel(
                name="Large City", description="A big city"
            ),
        ...
        self.assertEqual(
            restored_campaign.initial_quests["quest_015"].title, "Quest 15"
        )
