...
class TestTTSIntegrationService:
    ...
    def mock_tts_service(self):
        ...
        mock.get_available_voices = Mock(return_value=[
    ...
    @pytest.fixture
    def mock_game_state_repo(self):
        """Create a mock game state repository."""
        return Mock()
    
    @pytest.fixture
    def tts_integration_service(self, mock_tts_service, mock_game_state_repo):
        """Create TTS integration service instance."""
        return TTSIntegrationService(mock_tts_service, mock_game_state_repo)
    
    def test_is_narration_enabled_with_game_state(self, tts_integration_service, mock_game_state_repo):
    ...
    def test_is_narration_enabled_no_game_state(self, tts_integration_service, mock_game_state_repo):
    ...
    def test_is_narration_enabled_disabled_in_game_state(self, tts_integration_service, mock_game_state_repo):
    ...
    def test_get_current_voice_from_game_state(self, tts_integration_service, mock_game_state_repo):
    ...
    def test_get_current_voice_default(self, tts_integration_service, mock_game_state_repo):
        """Test default voice when no game state."""
        mock_game_state_repo.get_game_state.return_value = None
        
        assert tts_integration_service.get_current_voice() == "af_heart"
        
    def test_set_narration_enabled_success(self, tts_integration_service, mock_game_state_repo):
    ...
    def test_set_narration_enabled_no_game_state(self, tts_integration_service, mock_game_state_repo):
    ...
    def test_generate_tts_for_message_success(self, tts_integration_service, mock_game_state_repo, mock_tts_service):
        ...
        result = tts_integration_service.generate_tts_for_message("Hello world", "msg123")
        
        assert result == "/path/to/audio.mp3"
        mock_tts_service.synthesize_speech.assert_called_once_with("Hello world", "af_heart")
        
    def test_generate_tts_narration_disabled(self, tts_integration_service, mock_game_state_repo, mock_tts_service):
    ...
    def test_generate_tts_no_tts_service(self, mock_game_state_repo):
    ...
    def test_generate_tts_empty_message(self, tts_integration_service, mock_game_state_repo, mock_tts_service):
    ...
    def test_get_available_voices(self, tts_integration_service, mock_tts_service):
    ...
    def test_get_available_voices_no_service(self, mock_game_state_repo):
    ...
    def test_preview_voice_success(self, tts_integration_service, mock_tts_service):
        """Test voice preview generation."""
        result = tts_integration_service.preview_voice("af_heart", "Test message")
        
        assert result == "/path/to/audio.mp3"
        mock_tts_service.synthesize_speech.assert_called_once_with("Test message", "af_heart")
        
    def test_preview_voice_default_text(self, tts_integration_service, mock_tts_service):
...
class TestTTSHierarchyBehavior:
    ...
    def test_hierarchy_game_state_takes_precedence(self, mock_dependencies):
        ...
        service = TTSIntegrationService(
            mock_dependencies['tts_service'],
            mock_dependencies['game_state_repo']
        )
        ...
        mock_dependencies['tts_service'].synthesize_speech.assert_called_with("Test", "am_adam")
    
    def test_runtime_toggle_only_affects_game_state(self, mock_dependencies):
        ...
        mock_dependencies['game_state_repo'].save_game_state.assert_called_once_with(game_state)
        # No calls to update campaign instance or template
        
    def test_game_state_initialization_from_hierarchy(self):
        """Test that game state is properly initialized from the hierarchy.
        
        This test documents the expected behavior when campaign_service
        creates a game state from the hierarchy.
        """
        # This is more of a documentation test showing expected behavior
        # The actual hierarchy logic is in campaign_service.py
        
        # Scenario 1: Template(True) -> Instance(None) -> GameState(True)
        template_settings = {'narration_enabled': True, 'tts_voice': 'af_heart'}
        instance_settings = {'narration_enabled': None, 'tts_voice': None}
        expected_game_state = {'narration_enabled': True, 'tts_voice': 'af_heart'}
        
        # Scenario 2: Template(True) -> Instance(False) -> GameState(False)
        template_settings2 = {'narration_enabled': True, 'tts_voice': 'af_heart'}
        instance_settings2 = {'narration_enabled': False, 'tts_voice': 'am_adam'}
        expected_game_state2 = {'narration_enabled': False, 'tts_voice': 'am_adam'}
        
        # Scenario 3: Template(False) -> Instance(True) -> GameState(True)
        template_settings3 = {'narration_enabled': False, 'tts_voice': 'af_heart'}
        instance_settings3 = {'narration_enabled': True, 'tts_voice': None}
        expected_game_state3 = {'narration_enabled': True, 'tts_voice': 'af_heart'}
        
        # These scenarios document the expected hierarchy behavior
...
class TestTTSErrorHandling:
    ...
    def test_is_narration_enabled_handles_exceptions(self, caplog):
    ...
    def test_set_narration_enabled_handles_save_errors(self, caplog):
    ...
    def test_generate_tts_handles_synthesis_errors(self, caplog):
