...
class TestCharacterTemplateUnifiedCRUD(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.test_character = CharacterTemplateModel(
            ...
            # Stats
            base_stats=BaseStatsModel(STR=8, DEX=16, CON=14, INT=18, WIS=12, CHA=11),
            ...
            feats=[
                TraitModel(
                    name="War Caster",
                    description="Advantage on CON saves for concentration, cast with hands full",
                ),
                TraitModel(
                    name="Alert",
            ...
            starting_equipment=[
                ItemModel(
                    id="spellbook",
                    name="Spellbook",
                    description="Your wizard spellbook",
                    quantity=1,
                ),
                ItemModel(
                    id="quarterstaff",
                ...
                ItemModel(
                    id="scholars_pack",
                    name="Scholar's Pack",
                    description="Includes books, ink, and parchment",
                    quantity=1,
                ),
                ItemModel(
                    id="robes",
                    name="Fine Robes",
                    description="Elegant wizard robes",
                    quantity=1,
                ),
                ItemModel(
                    id="dagger", name="Dagger", description="A small blade", quantity=2
                ),
                ItemModel(
                    id="potion_healing",
    ...
    def test_create_character_with_all_fields(self) -> None:
        ...
        dagger = next(
            (
                item
                for item in saved_data["starting_equipment"]
                if item["name"] == "Dagger"
            ),
            None,
        )
        self.assertIsNotNone(dagger)
        assert dagger is not None  # Type guard for mypy
    ...
    def test_read_character_preserves_all_fields(self) -> None:
        ...
        dagger = next(
            (
                item
                for item in loaded_character.starting_equipment
                if item.name == "Dagger"
            ),
            None,
        )
    ...
    def test_update_character_all_fields(self) -> None:
        ...
        character.personality_traits[1] = (
            "I often get lost in my own thoughts and contemplations"
        )
    ...
    def test_metadata_includes_all_relevant_fields(self) -> None:
        ...
        # Check that subrace and subclass are present
        self.assertEqual(metadata.subrace, "High Elf")
        self.assertEqual(metadata.subclass, "School of Evocation")
        self.assertEqual(metadata.background, "Sage")

    def test_character_without_optional_fields(self) -> None:
