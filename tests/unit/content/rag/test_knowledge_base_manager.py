...
# Create a mock for HuggingFaceEmbeddings to avoid numpy compatibility issues
class MockHuggingFaceEmbeddings:
    ...
    def embed_query(self, text: str) -> List[float]:
        ...
        return np.random.rand(384).tolist()  # type: ignore[no-any-return]


# Patch before importing to avoid initialization issues
import app.content.rag.knowledge_base

app.content.rag.knowledge_base._HuggingFaceEmbeddings = MockHuggingFaceEmbeddings
...
class TestKnowledgeBaseManager(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        os.makedirs(
            os.path.join(cls.temp_dir, "app/content/data/knowledge/lore"), exist_ok=True
        )
        ...
        lore_path = os.path.join(
            cls.temp_dir, "app/content/data/knowledge/lore/generic_fantasy_lore.json"
        )
